(function() {
  'use strict';
  angular.module('restauranteurApp').controller('CreateEditItemCtrl', CreateEditItemCtrl);
  ...
  function CreateEditItemCtrl($scope, $sce, $state, $mdDialog, item, ItemService, $mdToast) {
   ...
   if(item != null){
     var itemTagList = item.tag_list.split(" ");
     var newTagList = "";
     for(var i=0; i< itemTagList.length; i++){
       newTagList += itemTagList[i].substr(1, itemTagList[i].length);
       if(i < itemTagList.length-1){
         newTagList += ", "
       }
     }
     CreateEditItemCtrlVM.item.tag_list = newTagList;
   }
