(function() {
  'use strict';
  angular.module('restauranteurApp').controller('ItemCtrl', ItemCtrl);
  ...
  function ItemCtrl($scope, $sce, $state, $mdDialog, items, $mdToast, ItemService) {
    ...
    ItemCtrlVM.filterItems = filterItems;
    ItemCtrlVM.itemPresent = ItemCtrlVM.items.length > 0;
    function filterItems(){
      ItemService.query({searchStr: ItemCtrlVM.search}).$promise.then(function(value) {
        ItemCtrlVM.items = value;
        ItemCtrlVM.itemPresent = ItemCtrlVM.items.length > 0;
